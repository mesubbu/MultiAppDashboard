...
vi.mock('@/components/dashboard/PageHeader', () => ({
  PageHeader: ({ title }: { title: string }) => <div>{title} header</div>,
}));
vi.mock('@/components/dashboard/MetricsCards', () => ({
  MetricsCards: mockMetricsCards,
}));
vi.mock('@/components/dashboard/AppManager', () => ({
...
beforeEach(() => {
  ...
  mockGetAppPage.mockResolvedValue({
    items: [
      {
        id: 'app_2',
        status: 'critical',
    ...
    pageInfo: { page: 2, pageSize: 20, totalItems: 2, totalPages: 1 },
  });
  mockGetTenants.mockResolvedValue({
    items: [{ id: 'tenant_1', name: 'Acme' }],
  });
});
...
describe('AppsPage', () => {
  it('builds app metrics and forwards parsed query props to AppManager', async () => {
    ...
    expect(mockMetricsCards).toHaveBeenCalledWith(
      expect.objectContaining({
        items: [
          expect.objectContaining({ label: 'Production services', value: '1' }),
          expect.objectContaining({
